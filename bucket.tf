...
resource "null_resource" "connector_distribution_download" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
            mkdir -p /tmp/distributions/${var.connector_name} || true && \
            rm /tmp/distributions/${var.connector_name}/${var.distribution_version}.${lower(var.distribution_content_type)} || true && \
            curl -s -L ${var.distribution_url} >> /tmp/distributions/${var.connector_name}/${var.distribution_version}.${lower(var.distribution_content_type)}
        EOT
  }
}
